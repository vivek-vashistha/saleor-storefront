...
const ProductPanel: React.FC<ProductPanelProps> = ({
	allBundles = [],
	messageType = "product_bundle_recommendation",
}) => {
	...
	return (
		<div className="relative flex h-full  flex-col overflow-auto rounded-r-xl">
			<header className="sticky top-0 z-30 flex items-center justify-between border-b border-border bg-header-foreground p-4">
				<div className="flex items-center gap-2">
					<h2 className="text-lg font-semibold">{panelTitle}</h2>
					{hasMultipleBundles && !isProductRecommendations && (
						<>
							<span className="rounded-md bg-background px-2 py-0.5 text-sm font-medium">
							...
							<Button
								variant="ghost"
								size="icon"
								className="hover:bg-primary/90 hover:text-background/90 ml-2 h-8 w-8 rounded-md border border-border bg-foreground text-background"
							...
							<Button
								variant="ghost"
								size="icon"
								className="hover:bg-primary/90 hover:text-background/90 h-8 w-8 rounded-md border border-border bg-foreground text-background"
