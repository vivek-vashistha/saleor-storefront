...
const ChatInput: React.FC<ChatInputProps> = ({ 
  ...
  const applyTag = useCallback((product: Product) => {
    if (!isTagging) return;
    ...
    if (e.key === 'Backspace' && taggedProducts.length > 0) {
      ...
      taggedProducts.forEach(taggedProduct => {
        ...
        if (curPos === productNameEndPos) {
          ...
          const newText = inputText.substring(0, productNameStartPos) + 
                         inputText.substring(productNameEndPos);
  ...
  return (
    <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          {taggedProducts.map((tag) => (
            <Badge 
              key={tag.id} 
              variant="default"
              className="px-2 py-0.5 text-sm font-medium flex items-center gap-1"
            >
              {tag.name}
              <button
      ...
      <div className="flex items-center gap-2 relative">
        ...
        size="icon"
        className="bg-[#0F172A] text-white hover:bg-[#0F172A]/90 transition-colors"
      ...
      >
        ...
        {isTagging && suggestions.length > 0 && (
          <div 
            ref={suggestionsRef}
            className="absolute bottom-full left-0 right-14 mb-1 bg-background rounded-md shadow-lg border border-border z-50 overflow-hidden"
            role="listbox"
            aria-label="Product suggestions"
          >
            <div className="text-xs font-semibold text-muted-foreground px-3 py-1.5 border-b border-border">
              Product Suggestions
            </div>
            ...
            <div className="max-h-[200px] overflow-y-auto">
              {suggestions.map((product, index) => (
                <div
                  ...
                  className={cn(
                    "px-3 py-2 text-sm cursor-pointer hover:bg-muted/60 transition-colors",
